...
while (not nash_equilibrium_reached):
    ...
    for agent in range(num_agents):
        ...
        _, agents_per_night = calculate_all_local_rewards(agents_chosen_night, k, b)
        current_local_reward = local_reward_function(agents_per_night[chosen_night - 1], b)
        ...
        for new_night in range(1, k + 1):
            if new_night != chosen_night:
                ...
                new_local_reward = new_local_rewards[new_night - 1]

                # Compare new local reward with the current local reward
                if new_local_reward > current_local_reward:
...
nash_equilibrium_df = pd.DataFrame({
    ...
    "Local_reward": final_local_rewards
})
